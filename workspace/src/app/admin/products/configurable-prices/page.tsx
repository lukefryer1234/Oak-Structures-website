...
import { Edit } from 'lucide-react'; // Removed PlusCircle, Trash2
// Dialog related imports are removed as the dialog functionality is removed.
...
export default function ConfigurablePricesPage() {
  ...
  const handleEditClick = (priceItem: ConfigurablePrice) => {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Edit prices for predefined combinations of Garage, Gazebo, and Porch options.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Table>
                  ...
                  <TableCell className="text-right">
                    {editingPriceId === priceItem.id ? (
                      <Input
                        ...
                        className="h-8 text-right w-28 bg-background" // Ensure input is styled for visibility
                        autoFocus
                        // Removed onBlur to rely on explicit Save/Cancel
                      />
                    ) : (
