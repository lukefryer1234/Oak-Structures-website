...
export async function fetchGalleryItemsAction(): Promise<GalleryItem[]> {
  try {
    ...
    querySnapshot.forEach((docSnap) => {
      ...
      if (parsed.success) {
        items.push({ id: docSnap.id, ...parsed.data });
      } else {
         console.warn(`Gallery item ${docSnap.id} failed validation:`, parsed.error.flatten().fieldErrors);
      }
    });
    return items;
  } catch (error) {
...
export async function updateGalleryOrderAction(
  ...
  try {
      ...
      let errorMessage = "Failed to update gallery order.";
