...
const orderSummary = {
  ...
  items: [ // Should match OrderItem schema in actions.ts
    ...
    { id: 'flooring1', name: 'Oak Flooring (25m²)', quantity: 1, price: 1875, description: 'Kilned Oak, 25m²', category: 'oak-flooring_COMPLETELY_DISABLED' }, // Path updated to reflect disabled state
      ...
      if (result.errors) {
        Object.entries(result.errors).forEach(([key, value]) => {
          if (Array.isArray(value) && value.length > 0) {
            if (key in form.getValues()) { // Check if key is a valid form field
                 form.setError(key as FieldPath<CheckoutFormValues>, { type: 'manual', message: value.join(', ') });
            } else if (key === 'form' && result.errors?.form) { // Handle general form error
