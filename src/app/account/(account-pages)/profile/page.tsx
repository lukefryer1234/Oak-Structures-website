...
export default function ProfilePage() {
  const { currentUser, setError, error, updateUserProfile } = useAuth();
  ...
  const handleProfileUpdate = async (
    ...
    try {
      ...
      // Update the user's profile with the name from the form
      await updateUserProfile(currentUser, { displayName: name });

      // No need to show a toast as updateUserProfile already does that
    } catch (error) {
  ...
  const handlePasswordChange = async (
    ...
    const currentPassword = (event.target as HTMLFormElement)[
      "current-password"
    ].value;
    const newPassword = (event.target as HTMLFormElement)["new-password"].value;
    const confirmPassword = (event.target as HTMLFormElement)[
      "confirm-password"
    ].value;
