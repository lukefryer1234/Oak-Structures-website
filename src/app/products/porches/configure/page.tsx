...
const porchConfig: CategoryConfig = {
  ...
  options: [
    ...
    },
    {
      ...
      type: "select",
      options: [
        { value: "floor", label: "Legs to Floor" },
        { value: "wall", label: "Legs to Wall" },
      ],
      defaultValue: "floor",
    },
    {
      ...
      type: "select",
      options: [
        { value: "narrow", label: "Narrow (e.g., 1.5m Wide)" },
        { value: "standard", label: "Standard (e.g., 2m Wide)" },
        { value: "wide", label: "Wide (e.g., 2.5m Wide)" },
      ],
...
const calculatePrice = (config: PorchConfig): number => {
...
export default function ConfigurePorchPage() {
  ...
  const [configState, setConfigState] = useState<PorchConfig>(() => {
    const initialState: Partial<PorchConfig> = {};
    categoryConfig.options.forEach((opt) => {
      initialState[opt.id as keyof PorchConfig] = opt.defaultValue as string;
    });
    return initialState as PorchConfig;
  });
  ...
  }, [configState]); // Recalculate price when configState changes

  const handleConfigChange = (id: string, value: string) => {
    setConfigState((prev: PorchConfig) => {
      const newState = { ...prev, [id as keyof PorchConfig]: value };
            ...
            <div className="space-y-6">
              {categoryConfig.options.map((option) => (
                <div key={option.id} className="text-center">
                  ...
                  {option.type === "select" && (
                    <Select
                      value={configState[option.id as keyof PorchConfig]}
                      onValueChange={(value) =>
