...
interface ConfigOption {
  ...
  defaultValue?: string | { area: number | string; length: string; width: string };
  unit?: string;
}
...
const oakFlooringConfig: CategoryConfig = {
  ...
  options: [
    ...
    // { id: 'thickness', label: 'Thickness', type: 'area', fixedValue: '20mm' }, // Removed fixed thickness display
    {
      ...
      type: "area",
      unit: "m²",
      defaultValue: { area: 10, length: "", width: "" },
    }, // Allows direct area or length*width
  ],
};
...
const calculateAreaAndPrice = (
  config: FlooringConfigState
): { area: number; price: number } => {
  const areaData = config.area || { area: 0, length: "", width: "" };
  let areaM2 = parseFloat(String(areaData.area)); // Ensure argument is a string
  ...
  const floorUnitPrice =
    config.oakType === "reclaimed" ? unitPrices.reclaimed : unitPrices.kilned;
...
export default function ConfigureOakFlooringPage() {
  ...
  const [configState, setConfigState] = useState<FlooringConfigState>(() => {
    const initialState: Partial<FlooringConfigState> = {};
    categoryConfig.options.forEach((opt) => {
      if (opt.id === "oakType") {
        initialState.oakType = opt.defaultValue as string;
      } else if (opt.id === "area") {
        initialState.area = opt.defaultValue as { area: string | number; length: string; width: string };
      }
    });
    // Assert that the constructed object conforms to FlooringConfigState.
    // This relies on categoryConfig.options providing a defaultValue for 'oakType' that is a string.
    return initialState as FlooringConfigState;
  });
  ...
  const handleConfigChange = (
    id: string, 
    value: string | { area: number | string; length: string; width: string }
  ) => {
    setConfigState((prev: FlooringConfigState) => ({ ...prev, [id]: value }));
  };
  ...
  const handleAreaChange = (
    ...
    setConfigState((prev: FlooringConfigState) => {
  ...
  const validateCurrentFlooring = (): {
    isValid: boolean;
    flooring?: Omit<FlooringListItem, "id">;
    ...
    if (area <= 0) {
      toast({
        ...
        title: "Invalid Area",
        description:
          "Please enter a valid area (either directly or via length and width).",
      });
  ...
  const handleAddToBasket = () => {
    ...
    console.log(
      "Adding single flooring item to basket (placeholder):",
      newBasketItem,
    );
    // --- End Placeholder ---
  ...
  const handleProceedToCheckout = () => {
    if (cuttingList.length === 0) {
      toast({
        ...
        title: "Empty List",
        description:
          "Please add at least one flooring area to the cutting list.",
      });
      return;
    }
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
                              <div className="text-center">
                                ...
                                <Input
                                  ...
                                  value={configState[option.id as "area"]?.area || ""}
                                  onChange={(e) =>
                                    handleAreaChange(e.target.value, "area")
                                  }
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                              <div className="text-center">
                                ...
                                <Input
                                  ...
                                  value={configState[option.id as "area"]?.length || ""}
                                  onChange={(e) =>
                                    handleAreaChange(e.target.value, "length")
                                  }
                                  className="mt-1 bg-background/70 text-center"
                                />
                              </div>
                              <div className="text-center">
                                ...
                                <Input
                                  ...
                                  step="any"
                                  placeholder="Calculate area"
                                  value={configState[option.id as "area"]?.width || ""}
                                  onChange={(e) =>
                                    handleAreaChange(e.target.value, "width")
                                  }
                                  className="mt-1 bg-background/70 text-center"
                                />
                              </div>
              ...
              <div className="space-y-6 border-t border-border/50 pt-6 mt-4">
                <div className="text-center space-y-2">
                  <p className="text-sm text-muted-foreground">
                    Estimated Price for this Area (excl. VAT & Delivery)
                  </p>
