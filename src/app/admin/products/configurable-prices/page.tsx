...
const initialPrices: ConfigurablePrice[] = [
  {
    id: "p1",
    category: "Garages",
    configKey: "medium-curved-2-nocat-reclaimed",
    configDescription:
      "Med Garage, Curved Truss, 2 Bay, No Cat Slide, Reclaimed Oak",
    price: 8500,
  },
  {
    id: "p2",
    category: "Garages",
    configKey: "large-straight-3-yescat-kilned",
    configDescription:
      "Lrg Garage, Straight Truss, 3 Bay, Cat Slide, Kilned Oak",
    price: 12000,
  },
  {
    id: "p3",
    category: "Gazebos",
    configKey: "4x4-curved-full-kilned",
    configDescription: "4x4 Gazebo, Curved Truss, Full Legs, Kilned Oak",
    price: 3500,
  },
  {
    id: "p4",
    category: "Porches",
    configKey: "standard-curved-floor-reclaimed",
    configDescription: "Std Porch, Curved Truss, Legs to Floor, Reclaimed Oak",
    price: 2150,
  },
];
...
const garageOptions = [
  ...
  "Truss: Curved/Straight",
  "Bays: 1-4",
  "CatSlide: Yes/No",
  "Oak: Reclaimed/Kilned",
];
const gazeboOptions = [
  "Legs: Full/Wall",
  "Size: 3x3/4x3/4x4",
...
export default function ConfigurablePricesPage() {
  ...
  const [editingPrice, setEditingPrice] = useState<ConfigurablePrice | null>(
    null,
  );
  ...
  const handleSavePrice = (event: React.FormEvent) => {
    ...
    if (
      ...
      !formState.category ||
      !formState.configKey ||
      !formState.configDescription
    ) {
      toast({
        // Use toast for validation message
        variant: "destructive",
        title: "Validation Error",
        description:
          "Please fill in all required fields (Category, Options, Price) and ensure the price is valid.",
      });
      return;
    }
    ...
    if (editingPrice) {
      ...
      setPrices((prev) =>
        prev.map((p) => (p.id === editingPrice.id ? newPriceEntry : p)),
      );
      ...
      toast({
        title: "Success",
        description: "New price configuration added.",
      });
    ...
    return options.map((opt) => (
      <div key={opt} className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Placeholder for selecting {opt.split(":")[0].toLowerCase()} options.
        </p>
      </div>
    ));
  };
    ...
    <Card>
      <CardHeader className="flex flex-row justify-between items-center">
        <div>
          ...
          <CardDescription>
            Set prices for specific combinations of Garage, Gazebo, and Porch
            options.
          </CardDescription>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
              <DialogTitle>
                {editingPrice ? "Edit" : "Add"} Price Configuration
              </DialogTitle>
              <DialogDescription>
                Define the price for a unique combination of product options.
                Ensure the combination is accurately described.
              </DialogDescription>
            </DialogHeader>
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    handleFormChange(
                      "category",
                      value as ConfigurablePrice["category"],
                    )
                  }
                  required
                >
              ...
              {formState.category && (
                <div className="p-4 border rounded-md bg-muted/50 space-y-4">
                  <p className="text-sm font-medium text-center text-muted-foreground">
                    Configuration Options (Placeholders)
                  </p>
                  <p className="text-xs text-center text-muted-foreground">
                    Note: Detailed option selection fields need implementation
                    here based on the chosen category.
                  </p>
              ...
              <div className="space-y-2 mt-4 p-3 bg-muted/50 rounded-md border">
                ...
                <p className="text-sm">
                  {formState.configDescription || "Configure options above..."}
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  Key: {formState.configKey || "-"}
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  Ensure this accurately reflects the selected options before
                  saving.
                </p>
              </div>
            </form>
            ...
            {prices.length > 0 ? (
              prices.map((price) => (
                <TableRow key={price.id}>
                  ...
                  <TableCell>
                    <span title={price.configKey} className="cursor-help">
                      {price.configDescription}
                    </span>
                  </TableCell>
