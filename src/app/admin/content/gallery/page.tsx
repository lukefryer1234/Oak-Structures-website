...
import { PlusCircle, Loader2 } from "lucide-react"; // Removed Trash2, Edit
...
export default function GalleryContentPage() {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      if (!formState.dataAiHint) {
        setFormState((prev) => ({
          ...prev,
          dataAiHint: file.name.split(".")[0].replace(/[-_]/g, " "),
        }));
      }
    }
  };
  ...
  const handleSaveItem = async (event: React.FormEvent) => {
    ...
    if (
      ...
      toast({
        ...
        description:
          "Image (URL or Upload), Alt Text, and AI Hint are required.",
      });
      setIsSaving(false);
      return;
    }
    ...
    if (selectedFile) {
      ...
      toast({
        title: "Info",
        description: "Image upload simulated. Using placeholder URL.",
      });
    }

    const itemDataToSave = {
      order:
        typeof formState.order === "number"
          ? formState.order
          : galleryItems.length > 0
            ? Math.max(...galleryItems.map((item) => item.order)) + 1
            : 1,
    ...
    let result;
    if (editingItem) {
      result = await updateGalleryItemAction({
        ...itemDataToSave,
        id: editingItem.id,
      });
    } else {
  ...
  // const onDragEnd = async ( // Unused variable
  //  result: any /* DropResult from react-beautiful-dnd */,
  // ) => {
  /* const handleDragEndPlaceholder = async ( // Remove unused function handleDragEndPlaceholder
  //  result: any / DropResult from react-beautiful-dnd /,
  // ) => {
    ...
    if (
      ...
      updateResult.item &&
      Array.isArray(updateResult.item)
    ) {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: updateResult.message || "Failed to update order.",
      });
      ...
      <div className="px-4 py-6 space-y-6 bg-white rounded-lg border shadow-sm">
        <div className="flex flex-row justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Add, edit, remove, and reorder images shown on the gallery page.
            </p>
          </div>
        ...
        <div>
          <p className="text-sm text-gray-500 mb-4">
            Drag and drop items to reorder them (Drag & Drop functionality is a
            placeholder).
          </p>
          ...
          {isLoading && galleryItems.length === 0 ? (
            ...
            <div className="space-y-4">
              {galleryItems.map((item) => (
                <div
                  key={item.id}
                  className="flex items-center p-4 gap-4 relative group bg-gray-50 rounded-lg border"
                >
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          ...
          >
            ...
            <div className="space-y-2">
              <Label htmlFor="gallery-alt-text">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  handleFormChange("order", parseInt(e.target.value, 10))
                }
                required
                disabled={isSaving}
              />
