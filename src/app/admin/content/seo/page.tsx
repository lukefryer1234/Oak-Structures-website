...
export default function SeoManagementPage() {
  ...
  const handleInputChange = (
    pageKey: string,
    field: "titleTag" | "metaDescription",
    value: string,
  ) => {
    setSeoData((prevData) =>
      prevData.map((page) =>
    ...
    if (result.success) {
      ...
      toast({
        ...
        description:
          result.message +
          (result.errors
            ? ` Details: ${result.errors.map((e) => `${e.path.join(".")} - ${e.message}`).join(", ")}`
            : ""),
      });
    }
  };
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Manage Title Tags and Meta Descriptions for key pages.
          </CardDescription>
        </CardHeader>
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>SEO Settings</CardTitle>
        <CardDescription>
          Manage the Title Tags and Meta Descriptions for key pages to improve
            ...
            >
              ...
              <AccordionContent className="pt-2 pb-4 space-y-4">
                <div className="space-y-1.5">
                  ...
                  />
                  <p
                    className={`text-xs ${getLengthColor(page.titleTag.length, MAX_TITLE_LENGTH)}`}
                  >
                    Length: {page.titleTag.length} / {MAX_TITLE_LENGTH}{" "}
                    recommended
                  </p>
                </div>

                <div className="space-y-1.5">
                  <Label htmlFor={`desc-${page.pageKey}`}>
                    Meta Description
                  </Label>
                  <Textarea
                  ...
                  />
                  <p
                    className={`text-xs ${getLengthColor(page.metaDescription.length, MAX_DESC_LENGTH)}`}
                  >
                    Length: {page.metaDescription.length} / {MAX_DESC_LENGTH}{" "}
                    recommended
                  </p>
                </div>
                <div className="mt-4 p-3 border rounded-md bg-muted/30 space-y-1">
                  ...
                  <p className="text-blue-700 text-lg truncate">
                    {page.titleTag || `Default ${page.pageName} Title`}
                  </p>
                  <p className="text-green-700 text-sm">
                    https://timberline.com/
                    {page.pageKey === "home"
                      ? ""
                      : page.pageKey.replace(/ /g, "-").toLowerCase()}
                  </p>
                  <p className="text-sm text-gray-700 line-clamp-2">
                    {page.metaDescription ||
                      `Default meta description for the ${page.pageName} page providing details about...`}
                  </p>
                </div>
              </AccordionContent>
            </AccordionItem>
